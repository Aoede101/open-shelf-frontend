...
import { Send, ArrowLeft, Users, Clock, MessageCircle, Loader } from 'lucide-react';
...
export default function DiscussionRoomPage() {
  ...
  const fetchDiscussion = async (silent = false) => {
    try {
      ...
      console.log('Discussion ID:', data._id);
      console.log('Participants:', data.participants?.map(p => ({ id: p._id, username: p.username })));
  ...
  const handleJoinDiscussion = async () => {
    if (!discussion?.book?._id) {
      alert('Cannot join discussion: Book information missing');
      return;
    }

    setIsJoining(true);
    console.log('=== ATTEMPTING TO JOIN ===');
    console.log('Book ID:', discussion.book._id);
    
    try {
      const updatedDiscussion = await discussionService.joinDiscussion(discussion.book._id);
      console.log('✅ JOIN SUCCESSFUL');
      console.log('Updated participants:', updatedDiscussion.participants?.map(p => ({ id: p._id, username: p.username })));
      ...
      alert(error.response?.data?.error || 'Failed to join discussion. Please try again.');
    } finally {
      setIsJoining(false);
    }
  };
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const updatedDiscussion = await discussionService.sendMessage(id, tempMessage);
      console.log('✅ Message sent, new count:', updatedDiscussion.messages?.length);
      setDiscussion(updatedDiscussion);
    } catch (error) {
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-4">Loading discussion...</p>
      </div>
    );
  }
  ...
  // FIXED: Proper participant check with detailed logging
  const currentUserId = user?.id;
  const participantIds = discussion.participants?.map(p => p._id || p.toString()) || [];
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center space-x-4">
            {discussion.book?.cover && (
              <img
                src={discussion.book.cover.startsWith('http') 
                  ? discussion.book.cover 
                  : `${import.meta.env.VITE_API_URL.replace('/api', '')}${discussion.book.cover}`
                }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md flex flex-col h-[600px]">
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          {discussion.messages && discussion.messages.length > 0 ? (
            discussion.messages.map((msg, index) => {
              ...
              const showAvatar = index === 0 || discussion.messages[index - 1]?.user?._id !== msg.user?._id;
        ...
        <div className="border-t dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-900">
          {isParticipant ? (
            ...
            // USER NOT PARTICIPANT - SHOW JOIN BUTTON
